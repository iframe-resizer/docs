...
common scenarios you should be aware of to avoid unexpected behaviors.
...
its content, these techniques are not necessary as the viewport will now come to meet the
...
as there will always be content at the bottom of the page.
...
Either your iframe will be blocked from downsizing, as the content size is now based on the
iframe size. Or potentially it can cause a resizing loop, where the iframe changing size causes
the content to change size, which then in turn triggers an update to the iframes size.

You should therefore only use relative or fixed sizing units, such as `em` and `px`.

### Overflowing Elements

When page elements are taken out of the default document flow, or overflow the document body,
they can potentially impact the size of the iframe and may need individually checking for
position to calculate the content size.

To mitigate having to check large numbers of page element, _iframe-resizer_ uses the browsers
Observer APIs to reduce the <span class="fig">Set</span> of checkable page elements as much as
possible, but you should still be mindful that an excessively large number of overflown elements
could have an impact on page performance.

Enabling the `log: 'expanded'` option, will output details of when overflown elements are detected
and used to calculate the size of the iframe to the
[console](https://developers.google.com/chrome-developer-tools/docs/console#opening_the_console).
