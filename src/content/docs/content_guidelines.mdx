...
When creating content for your iframe, the vast majority of pages will work without any
changes. This page details a few minor considerations you should be aware of to avoid
unexpected behavers.  

### Footers

Many websites, including this site's homepage, have page footers anchored to the bottom of 
...
element, _iframe-resizer_ uses the browsers Observer APIs to reduce the `set` of checkable
page elements as much a possible, but you should still be mindful that an excessively large
number of overflown elements could have an impact on page performance.

Enabling the `log:true` option, will output details of when overflown elements are detected
