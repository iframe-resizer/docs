...
The _iframe-resizer_ project is composed of separate JS files for the parent and child pages.
...
needs to be included on every page shown in your iframe.
...
Setting the initial height of the iframe to `100vh` makes loading appear smoother to the user, as they will only see the content
...
The child page needs to load its own JavaScript package to enable communication
with the parent page. This is a small self-contained script with no dependencies.
It is designed to be a good guest on someone else's site and will only start running
