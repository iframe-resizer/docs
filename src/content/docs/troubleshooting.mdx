...
When the iframe has difficulty downsizing the root cause will normmally be one of two different types of CSS issue. The most 
common cause is having absolutely positioned elements anchored to the bottom or left side of the page. The other common cause 
is using Viewport or Percentage Sizing Units on the same axes that _iframe-resizer_ is controlling. The 
[Iframe Content Guidelines](/content_guidelines) section explains both of these issues and how to mitigate them.
...
use for page size calculation with the `data-iframe-size` attribute. The [Advanced Setup](/setup/advanced) guide explains 
