...
This method is disabled. To remove the iframe from the page, use Solid's
conditional rendering (`<Show>` or similar) to remove the component.

### disconnect()

This method is not required. _Iframe-resizer_ is disconnected automatically
via Solid's `onCleanup` lifecycle hook when the component is removed.
