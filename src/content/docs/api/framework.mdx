...
The `@iframe-resizer/core` package provides a [High Order Function](https://eloquentjavascript.net/05_higher_order.html), that allows you to configure _iframe-resizer_ and then connect a single [HTMLIFrameElement](https://developer.mozilla.org/en-US/docs/Web/API/HTMLIFrameElement). The package is written in TypeScript and re-exports every public type and constant — see the [Types](/api/types) page for the full list.

```ts
import connectResizer, { type IFrameOptions } from '@iframe-resizer/core'

const options: IFrameOptions = { license: 'xxxx' }
const resizer = connectResizer(options)(HTMLIFrameElement)
```

The returned `resizer` object is an [`IFrameObject`](/api/types#iframeobject) and exposes the
[Parent Page API methods](/api/parent#methods). This provides the means to disconnect _iframe-resizer_
from the iframe, to enable it to be safely removed from the page.
...
For frameworks, such as React, that make use of a Virtual DOM, there are a couple of considerations.
...
Before having a Virtual DOM remove an iframe from the page, _iframe-resizer_ needs to be disconnected from the iframe first. The Parent Page API provides a [disconnect()](/api/parent#disconnect) method to unbind _iframe-resizer_ from the iframe element.
...
The full [React component](https://github.com/davidjbradshaw/iframe-resizer/blob/master/packages/react/index.tsx) further demonstrates integration with the [Parent Page API](/api/parent).
