...
[Options](#options) contains settings that can be passed when calling iframe-resizer. [Events](#events) are 
...
The following options can be set from within the iframe page by creating a `window.iframeResizer` object before 
the JavaScript file is loaded into the page.
...
The following events can be included in the [options](#options) object attached to the iframed page.
