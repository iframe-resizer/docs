...
parent page. Pass a single origin as a string, or an array of allowed origins to support multiple parent
domains. This option is really a last line of defense, and you should also set up a
...
[`IFrameParentProps`](/api/types#iframeparentprops) object with the following read-only properties:
...
```js
parentIframe.getVersion()
// { parent: '6.0.0', child: '6.0.0' }
```

If the parent page is running an older version of _iframe-resizer_ that does not report its version, the
`parent` property will be `'legacy'`.
