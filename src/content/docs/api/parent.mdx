...
**Open Source** If you are using _iframe-resizer_ on an open-source site or project, then you can use it for free under the terms of the [Gnu Public License V3](/gpl). To accept these terms set the license to `GPLv3`.
...
<Aside type="tip" title="Horizontal Resizing">
  ...
  positioning elements to the right side of the page, as this can interfer with
...
that is consitantly slightly different to how you want the iframe to be sized.
...
```js
onScroll: ({ top, left, iframeId }) => [true | false];
```
...
The following example shows how to adjust where the scroll point ends up. This can be useful if you have a sticky header on your page and you need prevent the target scroll point ending up under your header.
```js
