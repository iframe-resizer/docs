...
These new packages are designed to be drop in replacements for the previous version and in most situations will work without further code changes. The parent and child pages can be upgraded independently from one another, as both packages are backwards compatible with previous versions.

Where the API has changed, the deprecated feature will trigger a waring in the browser console with advise on how to update your code.
...
This library has always supported resizing in both directions, but changing the direction confusingly required the setting of multiple different options in the config. This has now been consolidated into the new single `direction` option, which can have the following values: `vertical`, `horizontal`, `both`, or `none`.
...
### New <span class="option">onBeforeResize()</span> event

The `onBeforeResize(newSize)` event allows you to modify or replace the content size value, before it is sent to the parent page. This method replaces being able to set a custom size function on `heightCalculationMethod` or `widthCalculationMethod` options in the iframe.
...
The `size()` method in the [Child Page API](/api/child) has been deprecated in favour of `resize()`. This brings the child page name in line with the Parent Page API. 

Use of `size()` will trigger a deprecation warning in the console.

### The <span class="option">onInit()</span> callback has been renamed to <span class="option">onReady()</span>

The `onInit()` callback in the [Parent Page API](/api/parent) has been deprecated in favour of `onReady()`. This brings the parent page and child page names for this event inline with each other. 

Use of `onInit()` will trigger a deprecation warning in the console.

### The <span class="option">onClose()</span> callback has been renamed to <span class="option">onBeforeClose()</span>

The `onClose()` callback in the [Parent Page API](/api/child) has been deprecated in favour of `onBeforeClose()`. 

Use of `onClose()` will trigger a deprecation warning in the console.

### The <span class="option">onClosed()</span> callback has been renamed to <span class="option">onAfterClose()</span>

The `onClosed()` callback in the [Parent Page API](/api/child) has been deprecated in favour of `onAfterClose()`. 

Use of `onClosed()` will trigger a deprecation warning in the console.
...
Dropping support for legacy browsers, has allowed version 5 to use the latest browser APIs to detect content changes, this greatly improves both performance and event detection, whilst also eliminating the issue of one change being picked up by multiple different browser events.
