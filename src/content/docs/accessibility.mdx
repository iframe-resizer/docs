...
to aid users navigating with assistive technology, such as a screen reader, by using the
[title attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/title) on
an `<iframe>` to label its contained content.
...
Where iframes are missing the `title` attribute, _iframe-resizer_ will automatically set it with
the text contained within the `<title>` element in the `<iframe>`. This ensures that the value is
always up to date and correctly reflects the page that is currently displayed within.
